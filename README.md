...
photos/source/              Local private photographs (ignored by Git)
...
tracks/source/              Local private GPX recordings (ignored by Git)
...
Original photographs are kept privately outside the public Git history. For a local rebuild, copy them temporarily into the ignored `photos/source/` directory using these category folders:
...
Only the privacy-safe generated files are part of the deployed website and should be committed. Original photographs must remain outside Git.

Supported source formats include HEIC, HEIF, HIF, JPEG, PNG and WebP.

## Managing voyage tracks

Keep original GPX recordings outside the public Git history. For a local rebuild, copy them temporarily into the ignored `tracks/source/` directory, grouped as `year/voyage-name/`. Each GPX file becomes one leg and each folder becomes a voyage. Rebuild the published map data with:

```bash
python3 tools/build_tracks.py --tolerance 20
```

The source recordings remain unchanged. Distance and elapsed time are calculated from the complete track; only simplified geometry, passage timestamps and statistics are published in `data/tracks.geojson`. Recording gaps remain separate through GeoJSON `MultiLineString` geometry. If no private GPX files are present, the generator refuses to overwrite the existing published atlas.
